...
\begin{frame}{\onslide<\fScheme->{Spinlock terminates under interleaving semantics}}
  ...
  \begin{columns}
    ...
    \begin{column}{0.5\linewidth}
      % \only<\fProg->{\upd{\showConcrete}{true}}
      \spinlockClientIIExpandedIter
      % \begin{tikzpicture}[xscale=1, yscale=1]
      %   \onslide<2-3>{
      %     \node (P1) at (0, 0) {\includegraphics[width=0.9\textwidth]{terminated.png}};
      ...
      \tikz[overlay,remember picture]{
        \only<\fTerm-\fHang>{
          \node (Pic) at ([xshift=0cm,yshift=-2cm]current page.center) {\includegraphics[width=0.9\textwidth]{terminated.png}};
        }
        \only<\fHang>{
          \node (Pic) at ([xshift=0cm,yshift=-3.2cm]current page.center) {\includegraphics[width=0.7\textwidth]{pending.png}};
        }
      }
    \end{column}
