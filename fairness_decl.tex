
% \begin{frame}{Memory fairness corresponds to prefix-finiteness of a \only<1>{{\colorbox{white}{relation}}}\only<2->{{\colorbox{colorFR!50}{relation}}}}
\begin{frame}

  \renewcommand{\hof}{2}
  \renewcommand{\vof}{1}
  % \begin{column}{0.4\linewidth}
  \onslide<2->{Declarative counterpart of}
  \raisebox{-0.5cm}{\scalebox{0.8}{\propSubtrace}}
  \onslide<2->{is needed}

  \vspace{1cm}

  \onslide<3->{

    \begin{columns}[T]

      \begin{column}{0.4\linewidth}
        % \raisebox{1cm}{\spinlockClientIIExpanded}
        \spinlockClientIIExpanded

        \vfill

        % \vspace{2cm}
      \end{column}

      \begin{column}{0.5\linewidth}
        \vspace{-0.5cm}
        
        \renewcommand{\hof}{2}
        \renewcommand{\vof}{1}
        \begin{tikzpicture}[xscale=2, yscale=0.8]
          \spinlockInfGraphEvents
          \spinlockInfGraphPO
          \spinlockInfGraphRF
          \spinlockInfGraphMO
          % \onslide<4->{\spinlockInfGraphFRComp}
          \onslide<4->{
            \spinlockInfGraphRB
            \node (R24) at (0 * \hof, -4 * \vof) {$\lFR \defeq \lRF^{-1};\lMO$};
          }        
        \end{tikzpicture}

        \pause

      \end{column}

    \end{columns}

    \begin{center}
    \onslide<5->{
      $\TSOdeclfair \defeq acyclic(\overline{\lPO} \cup \lRF \cup \lMO)\land \ldots \land {\color{\colorTitleApprox}\underline{prefix\text{-}finite(\lFR)}}$
    }
  \end{center}

  \onslide<6->{
  \tikz[overlay,remember picture]{
    \node (Accepted) at ([xshift=0cm,yshift=-3.2cm]current page.center) {\includegraphics[width=0.3\textwidth]{approved.png}};
  }
  }

    % \tikz[overlay,remember picture]{
    %   % \draw[frComp,bend right=120] (7, -1) edge node[] { } (7, 5);
    % \myrectMulti{Allowed}{(12, 5)}{And now it's forbidden}{4cm}{1cm}{white};
    % }
  }
\end{frame}

%%% Local Variables:
%%% mode: latex
%%% TeX-master: "oopsla"
%%% End:
